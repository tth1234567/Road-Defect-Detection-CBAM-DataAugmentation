...
class SE(nn.Module):
    ...
    # r: int = 8：缩放比例 reduction ratio，默认 8。用来把通道数压缩成更小的中间维度。等号 = 8 表示有默认值，如果你不传，就用 8。
...
class C2fWithCBAM(nn.Module):
    def __init__(self, c2f_mod: C2f, r: int = 8, k: int = 7):
        super().__init__()
        self.c2f = c2f_mod
...
def main():
    ...
    parser.add_argument("--name", type=str, default="yolov8_cbam_c2f_ris_8_high_ch_is224_new")
