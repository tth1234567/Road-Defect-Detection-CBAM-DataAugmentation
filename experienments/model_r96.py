...
class SE(nn.Module):
    ...
    # r: int = 16：缩放比例 reduction ratio，默认 16。用来把通道数压缩成更小的中间维度。等号 = 16 表示有默认值，如果你不传，就用 16。
    def __init__(self, c: int, r: int = 96):
...
class CBAM(nn.Module):
    def __init__(self, c: int, r: int = 96, k: int = 7):
        super().__init__()
        '''
...
class C2fWithCBAM(nn.Module):
    def __init__(self, c2f_mod: C2f, r: int = 96, k: int = 7):
        super().__init__()
        self.c2f = c2f_mod
...
def replace_c2f_with_cbam(module: nn.Module,
                          ...
                          high_ch_threshold: int = 192) -> Tuple[int, int]:
    """
...
def main():
    ...
    parser.add_argument("--name", type=str, default="yolov8_cbam_c2f_ris_96")
    ...
    parser.add_argument("--high_ch_threshold", type=int, default=96,
