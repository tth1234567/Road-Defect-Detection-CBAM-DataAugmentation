...
export function getTrainingCurves() {
  return http.get('/training-curves')
}

export function getABImages() {
  return http.get('/ab-images')
}

export function detectBaseline(imageFile, confidence = 0.25) {
  const formData = new FormData()
  formData.append('file', imageFile)
  formData.append('confidence', confidence)
  return http.post('/detect/baseline', formData, {
    headers: { 'Content-Type': 'multipart/form-data' },
  })
}

export function detectImproved(imageFile, confidence = 0.25) {
  const formData = new FormData()
  formData.append('file', imageFile)
  formData.append('confidence', confidence)
  return http.post('/detect/improved', formData, {
    headers: { 'Content-Type': 'multipart/form-data' },
  })
}
